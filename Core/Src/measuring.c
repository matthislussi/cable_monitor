...
static uint32_t sequence_cntr = 0;
...
static uint32_t (*ADC_average)[4*ADC_NUMS]; ///< ADC values of 2 inputs with each 7 conversions
 ...
 * @n As each conversion triggers the DMA, the number of transfers is doubled.
 * @n The DMA triggers the transfer complete interrupt when all data is ready.
 * @n The inputs used are: ADC123_IN13 = GPIO PC3 (PAD L), ADC3_IN4 = GPIO PF6 (PAD R),
...
void DMA2_Stream1_IRQHandler(void)
{
	if (DMA2->LISR & DMA_LISR_TCIF1) {	// Stream1 transfer compl. interrupt f.
		...
		ADC_reset();
		if (sequence_cntr >= SEQUENZ_MAX){
			ADC_average = samples_mean();
			MEAS_data_ready = true;
			sequence_cntr = 0;
		} else {
			sequence_cntr++;
			samples_add(ADC_samples);
		}
	}
}
...
void MEAS_show_data(void)
{
	...
	BSP_LCD_SetTextColor(LCD_COLOR_BLUE);
	data = (*ADC_average)[MEAS_input_count*0] / f;
	for (uint32_t i = 1; i < ADC_NUMS; i++){
		data_last = data;
		data = ((*ADC_average)[MEAS_input_count*i]) / f;
	...
	if (MEAS_input_count >= 2) {
		...
		data = (*ADC_average)[MEAS_input_count*0+1] / f;
		for (uint32_t i = 1; i < ADC_NUMS; i++){
			data_last = data;
			data = ((*ADC_average)[MEAS_input_count*i+1]) / f;
			if (data > Y_OFFSET) { data = Y_OFFSET; }// Limit value, prevent crash
			BSP_LCD_DrawLine(4*(i-1), Y_OFFSET-data_last, 4*i, Y_OFFSET-data);
		}
	}
	/* Clear buffer and flag */
	for (uint32_t i = 0; i < (MEAS_input_count*ADC_NUMS)-1 ; i++){
		ADC_samples[i] = 0;
		(*ADC_average)[i] = 0;
	}
	ADC_sample_count = 0;
